 ...
 * 
 * Usage: npx ts-node scripts/export-god-status-schema-information-schema.ts
 */
...
/**
 * Export god_status table schema via agent_exec_sql
 * Query: information_schema.columns and information_schema.tables
 * No row filters, no aggregations - pure schema introspection
 */
async function exportGodStatusSchemaViaInformationSchema(): Promise<void> {
  ...
  try {
    ...
    const exportFile = path.join(exportDir, 'god-status-schema-export.json')

    fs.writeFileSync(exportFile, JSON.stringify(exportResult, null, 2))
    console.log(`\n✅ Export saved to: ${exportFile}`)

    // ===== Export to Markdown file =====
    const markdownFile = path.join(exportDir, 'god-status-schema-export.md')
...
function generateMarkdownReport(exportResult: TableSchemaExport): string {
  ...
  exportResult.columns.forEach((col) => {
    ...
    const charMax = col.character_maximum_length ? String(col.character_maximum_length) : '-'
    const numPrec = col.numeric_precision ? String(col.numeric_precision) : '-'
    
    md += `| ${col.ordinal_position} | ${col.column_name} | ${col.data_type} | ${nullable} | ${defaultVal} | ${charMax} | ${numPrec} |\n`
  })
  ...
  md += `- **Function Used**: \`agent_exec_sql()\`\n`
  md += `- **Schema Source**: PostgreSQL \`information_schema\` catalog tables\n`
  md += `- **Query Type**: Direct column and table metadata (no filters, no aggregations)\n`
  md += `- **RPC Method**: Supabase RPC with service role authentication\n`
  md += `- **Result Format**: JSON array\n\n`

  md += `## Notes\n\n`
  md += `This export uses PostgreSQL's \`information_schema\` views to retrieve complete table and column metadata.\n`
  md += `No row-level filters or aggregations are applied - this is pure schema introspection.\n\n`
